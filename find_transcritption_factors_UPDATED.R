...
dist_0 <- net_dist_bin(prob_matrix_grn,TF_list,thr=1)
tf_network_summary <- net_summary_bin(prob_matrix_grn,TF_list)
summary_GRN_table <- as.data.frame(tf_network_summary$GRN_table)
# calculate the closeness of a matrix
summary_closeness <- tf_network_summary$Close_centr
# non-trivial close values
summary_closeness <- summary_closeness[summary_closeness>0]

# we need to revisit this. the closeness matrix gave the distance between each TF and its targets

grn_TFs_filtered %>% 
  select(
        grn_TFs_filtered %>% 
        summarise(across(max)) %>% 
        pivot_longer(everything()) %>% 
        filter(value > 0.2) %>% 
        pull(name)
    )
dummy<- gather(dist_chosen_tf_targets, protein, interaction, PRDM2:TFE3, factor_key=TRUE)
